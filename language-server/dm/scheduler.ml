...
type skip = { id: sentence_id; error: Pp.t option }
...
let push_Query_in_block ex_sentence block =
  { block with proof_sentences = E ex_sentence :: block.proof_sentences }
let push_Query ex_sentence st =
  match st.proof_blocks with
  | [] -> { st with document_scope = ex_sentence.id :: st.document_scope }, Query ex_sentence
  | l::q -> { st with proof_blocks = push_Query_in_block ex_sentence l :: q }, Query ex_sentence
